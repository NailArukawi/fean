    ...
    pub fn debug(self: *@This(), fn_print: bool) void {
        for (self.code.items, 0..) |instr, i| {
            ...
            switch (op) {
                ...
                .ret => std.debug.print("[{}]:\t[{s}]\t\t({})\n", .{ i, @tagName(op), instr.z.a > 0 }),
                .call => std.debug.print("[{}]:\t[{s}]\t\t(reg[{}]() -> reg[{}])\n", .{ i, @tagName(op), instr.z.c, instr.z.a }),
                ...
                .if_jmp => std.debug.print("[{}]:\t[{s}]\t(if(reg[{}]) ip += {})])\n", .{ i, @tagName(op), instr.y.a, @as(i20, @bitCast(instr.y.y)) }),
                .jmp => std.debug.print("[{}]:\t[{s}]\t\t(ip += {})])\n", .{ i, @tagName(op), @as(i30, @bitCast(instr.x.x)) }),
